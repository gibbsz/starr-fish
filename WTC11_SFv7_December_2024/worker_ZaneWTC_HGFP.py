...
save_folder =r'D:\241207_WTC11_SFv7_HMERFISH_Analysis'  ###change
...
if __name__ == '__main__':
    items = [(set_,ifov) for set_ in ['']
                        for ifov in range(184)]

    inds = np.random.permutation(np.arange(len(items)))###
    items = [items[i] for i in inds]####
       
    
    #main_f(('',30),redo_drift=False,redo_decoding=False,try_mode=False)
    if True:
        with Pool(processes=4) as pool:
            print('starting pool')
            result = pool.map(main_f, items)
#activate cellpose2&&python C:\Users\zgibbs\Scripts\worker_ZaneWTC_HGFP.py