...
cell_type_counts = pd.DataFrame(index=cell_types_counts2.index.intersection(cell_types_counts1.index).intersection(starrfish3.get_celltypes()), columns=['Sec1', 'Sec2', 'Exp3'])
...
def plot_heatmap(expression_mat, cell_type_label, cell_types_to_use, cre_anno, scale=None, log=False):
    ...
    # Reorder columns by library size
    ...
    sns.heatmap(np.log10(starrfish3.lib_size['counts'].loc[col_order].values.reshape(1, -1)),
                cmap='coolwarm', ax=ax[1], cbar_kws={'label': 'log10(Library Size)'}, yticklabels=['Library Size'])
    ax[1].set_xticks([])
...
fig = plot_heatmap(starrfish3.get_cre_expression(), cell_type_labels, cell_types_to_use, cre_anno, scale=1000, log=True)
fig.savefig('results/expr3/cre_counts_per_celltype_heatmap.pdf')
fig = plot_heatmap(starrfish3.get_t7_expression(), cell_type_labels, cell_types_to_use, cre_anno, scale=100, log=True)
fig.savefig('results/expr3/t7_counts_per_celltype_heatmap.pdf')
fig = plot_heatmap(starrfish3.get_cre_expression() > 0, cell_type_labels, cell_types_to_use, cre_anno, scale=1000, log=True)
fig.savefig('results/expr3/cre_percentage_per_celltype_heatmap.pdf')
fig = plot_heatmap(starrfish3.get_t7_expression() > 0, cell_type_labels, cell_types_to_use, cre_anno, scale=100, log=True)
