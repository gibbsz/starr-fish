...
# %%
class T7CRE_bin_DistributionEM:
    def __init__(self, device='cuda' if torch.cuda.is_available() else 'cpu', use_x0=True, clamp=True):
        self.device = device
        self.use_x0 = use_x0
        self.clamp = True
    
    def expectation_step(self, x0, x1, x2, obs_t7, obs_cre, cell_type_indexes, dim=2):
        ...
        if self.clamp:
            k_lambda = torch.sigmoid(torch.clamp(x0[cell_type_indexes], min=-100, max=100))  # Clamp to prevent overflow
        else:
            k_lambda = torch.sigmoid(x0[cell_type_indexes])
        ...
        # k ~ Bernoulli(k_lambda): log P(k|k_lambda)
        if self.use_x0:
            ll_k = torch.distributions.Bernoulli(k_lambda_expanded).log_prob(k_values_expanded)  # (n_obs, dim)
        else:
            ll_k = 0  # No contribution from k
        # Vectorized obs_t7 ~ NegBinom(exp(x1[0]) * k, torch.sigmoid(x1[1]))
        if self.clamp:
            detection_rate_t7 = torch.sigmoid(torch.clamp(x1, min=-100, max=100)) # Apply constraint
        else:
            detection_rate_t7 = torch.sigmoid(x1)
        mu_t7 = detection_rate_t7 * (k_values + 1e-8)  # (dim,)
        mu_t7_expanded = mu_t7.unsqueeze(0)  # (1, dim)
        
        # T7 log PMF computation
        ll_t7 = torch.distributions.Bernoulli(mu_t7_expanded).log_prob(obs_t7_expanded)  # (n_obs, dim)
        
        # Vectorized obs_cre ~ NegBinom(exp(x2[cell_type_indexes, 0]) * k, torch.sigmoid(x2[cell_type_indexes, 1]))
        if self.clamp:
            detection_rate_cre = torch.sigmoid(torch.clamp(x2[cell_type_indexes], min=-100, max=100))  # (n_obs,)
        else:
            detection_rate_cre = torch.sigmoid(x2[cell_type_indexes])
        mu_cre = detection_rate_cre.unsqueeze(1) * k_values.unsqueeze(0)  # (n_obs, dim)

        # CRE log PMF computation
        ll_cre = torch.distributions.Bernoulli(mu_cre).log_prob(obs_cre_expanded)  # (n_obs, dim)
    ...
    def maximization_step_x0(self, x0, cell_type_indexes, posterior_k):
        ...
        def closure():
            ...
            if self.clamp:
                x0_clamped = torch.clamp(x0, min=-100, max=100)
            else:
                x0_clamped = x0
            # Get lambda values for each observation based on cell type
            k_lambda = torch.sigmoid(x0_clamped[cell_type_indexes])  # (n_obs,)
            k_lambda_expanded = k_lambda.unsqueeze(1)  # (n_obs, 1)
            k_values_expanded = k_values.unsqueeze(0)  # (1, dim)

            # k ~ Bernoulli(k_lambda): log P(k|k_lambda)
            ll_k = torch.distributions.Bernoulli(k_lambda_expanded).log_prob(k_values_expanded)
    ...
    def fit(self, cell_types, obs_t7, obs_cre, dim=1000, max_iter=50):
        ...
        # convert obs_t7 and obs_cre to binary
        obs_t7 = (obs_t7 > 0).astype(np.float32)
        obs_cre = (obs_cre > 0).astype(np.float32)
        ...
        for i, ct in enumerate(unique_celltypes):
            ...
            x0_init.append(np.log(zero_frac))

        x0 = torch.tensor(x0_init, device=self.device, dtype=torch.float32)
        # Clamp initial x0 to reasonable range
        if self.clamp:
            x0 = torch.clamp(x0, min=-100, max=100)
        
        x1 = torch.tensor([0], device=self.device, dtype=torch.float32)  # Start with more conservative value
        x2 = torch.tensor(np.zeros(len(unique_celltypes)), device=self.device, dtype=torch.float32)  # Start with more conservative value
...
for cre in t7_counts_sec1.columns:
    ...
    x2_df_sec1[cre] = x2_sec1
    x2_df_sec2[cre] = x2_sec2
# %%
x0_df_sec1.to_csv(f'{PWD}/results/expr3/t7cre_bin_mle_separate.x0.sec1.csv')
x0_df_sec2.to_csv(f'{PWD}/results/expr3/t7cre_bin_mle_separate.x0.sec2.csv')
x1_df_sec1.to_csv(f'{PWD}/results/expr3/t7cre_bin_mle_separate.x1.sec1.csv')
x1_df_sec2.to_csv(f'{PWD}/results/expr3/t7cre_bin_mle_separate.x1.sec2.csv')
x2_df_sec1.to_csv(f'{PWD}/results/expr3/t7cre_bin_mle_separate.x2.sec1.csv')
x2_df_sec2.to_csv(f'{PWD}/results/expr3/t7cre_bin_mle_separate.x2.sec2.csv')
# %% analysis
x0_df_sec1 = pd.read_csv(f'{PWD}/results/expr3/t7cre_bin_mle_separate.x0.sec1.csv', index_col=0)
x0_df_sec2 = pd.read_csv(f'{PWD}/results/expr3/t7cre_bin_mle_separate.x0.sec2.csv', index_col=0)
x1_df_sec1 = pd.read_csv(f'{PWD}/results/expr3/t7cre_bin_mle_separate.x1.sec1.csv', index_col=0)
x1_df_sec2 = pd.read_csv(f'{PWD}/results/expr3/t7cre_bin_mle_separate.x1.sec2.csv', index_col=0)
x2_df_sec1 = pd.read_csv(f'{PWD}/results/expr3/t7cre_bin_mle_separate.x2.sec1.csv', index_col=0)
x2_df_sec2 = pd.read_csv(f'{PWD}/results/expr3/t7cre_bin_mle_separate.x2.sec2.csv', index_col=0)
# %%
cre_corr, celltype_corr = starrfish3_sec1.corr_starrfish(x2_df_sec1, x2_df_sec2)
