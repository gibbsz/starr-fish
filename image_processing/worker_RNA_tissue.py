...
def get_best_translation_pointsT(fl,save_folder,fl_ref,save_folder_ref,set_='',resc=5,th=5,
    ...
    if (len(obj.Xh_plus)>0) and (len(obj_ref.Xh_plus)>0):
        ...
        try:
            ...
            tzxy_plus,N_plus = np.array([0,0,0]),-1
    if (len(obj.Xh_minus)>0) and (len(obj_ref.Xh_minus)>0):
        ...
        try:
            ...
            tzxy_minus,N_minus = np.array([0,0,0]),-1
        #tzxy_minus,N_minus = get_best_translation_points(X,X_ref,resc=resc,return_counts=True)
...
def main_do_compute_fits(save_folder,fld,fov,icol,save_fl,psf,old_method):
    ...
    Xh = get_local_max_tile(im__,th=3600,s_ = 280,pad=100,psf=psf,plt_val=None,snorm=50,gpu=True,
...
def compute_decoding(save_folder,fov,set_,redo=False):
    ...
    if complete==0 or redo:
        ...
        dec.get_XH(fov,set_,ncols=3,th_h=7500,tag_keep='MER',medH_fl=None,color_fl=r'C:\Scripts\NMERFISH\ZaneS6A4\color_correction_3col_scope5_V2.pkl')#number of colors match 
        ...
        dec.ncols = 3
        get_intersV2(dec,nmin_bits=3,dinstance_th=1.5,enforce_color=False,enforce_set=None,redo=False)
...
def get_best_translation_pointsV2(fl,fl_ref,save_folder,set_,resc=5,th=4):
    ...
    if (len(obj.Xh_plus)>0) and (len(obj_ref.Xh_plus)>0):
        ...
        try:
            ...
            tzxy_plus,N_plus = np.array([0,0,0]),0
    if (len(obj.Xh_minus)>0) and (len(obj_ref.Xh_minus)>0):
        X = obj.Xh_minus#[:,:3]
        ...
        try:
            ...
            tzxy_minus,N_minus = np.array([0,0,0]),0
    if np.max(np.abs(tzxy_minus-tzxy_plus))<=2:
        tzxyf = -(tzxy_plus*N_plus+tzxy_minus*N_minus)/(N_plus+N_minus)
    else:
        tzxyf = -[tzxy_plus,tzxy_minus][np.argmax([N_plus,N_minus])]
...
if __name__ == '__main__':
    ...
    main_f(['_set2',143],redo_fits =False,redo_drift=False,redo_decoding=False,try_mode=False)#49,560
    
    if True:
        with Pool(processes=4) as pool:
            print('starting pool')
            result = pool.map(main_f, items)
#python worker_RNA_tissue.py